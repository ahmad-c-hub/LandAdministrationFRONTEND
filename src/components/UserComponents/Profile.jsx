...
const Profile = () => {
  ...
  const fetchLogs = async (pageNumber) => {
    try {
      const res = await axios.get(`http://localhost:8080/user-log/current-user?page=${pageNumber}&size=10`);
  ...
  const handleUsernameChange = async () => {
    ...
    try {
      await axios.put("http://localhost:8080/user/update-current-user", {
